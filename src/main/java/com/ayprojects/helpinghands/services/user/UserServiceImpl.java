...
public class UserServiceImpl implements UserService {
    ...
    public Response<DhUser> addUser(HttpHeaders httpHeaders, DhUser dhUserDetails, String version) {
        ...
        if (queriedUser.isPresent()) {
            LOGGER.info("UserServiceImpl->Found user with mobileNumber");
            return new Response<>(false, 402, Utility.getResponseMessage(AppConstants.RESPONSEMESSAGE_USER_ALREADY_EXISTS_WITH_MOBILE, language), new ArrayList<>());
        } else {
            queriedUser = userDao.findByEmailId(dhUserDetails.getEmailId());
            if (queriedUser.isPresent()) {
                LOGGER.info("UserServiceImpl->Found user with mobileNumber");
                return new Response<>(false, 402, Utility.getResponseMessage(AppConstants.RESPONSEMESSAGE_USER_ALREADY_EXISTS_WITH_EMAIL, language), new ArrayList<>());
            }
        }
    ...
    @Override
    public Response<DhUser> getUserByMobile(HttpHeaders httpHeaders, String mobileNumber, String countryCode, String version) {
        String language = Utility.getLanguageFromHeader(httpHeaders).toUpperCase();
        LOGGER.info("language=" + language);
