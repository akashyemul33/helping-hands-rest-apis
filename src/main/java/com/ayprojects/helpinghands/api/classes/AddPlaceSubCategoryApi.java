...
public class AddPlaceSubCategoryApi implements AddBehaviour<PlaceSubCategories> {
    @Override
    public Response<PlaceSubCategories> add(String language, MongoTemplate mongoTemplate, PlaceSubCategories placeSubCategory) {
        ...
        if (returnResponse.getStatus()) {
            ...
            DhPlaceCategories queriedDhPlaceCategories = getMainCategory(placeSubCategory.getPlaceMainCategoryId(), mongoTemplate);
            ...
            boolean isSubCategoryAlreadyExists = checkWhetherSubCategoryAlreadyExists(queriedDhPlaceCategories.getPlaceSubCategories(), placeSubCategory.getDefaultName());
            ...
            queriedDhPlaceCategories.getPlaceSubCategories().add(placeSubCategory);

            updateMainCategory(placeSubCategory, mongoTemplate);
            returnResponse.setLogActionMsg("New sub category [" + placeSubCategory.getDefaultName() + "] has been added under [" + queriedDhPlaceCategories.getDefaultName() + "].");
        }
    ...
    private void updateMainCategory(PlaceSubCategories placeSubCategory, @NonNull MongoTemplate mongoTemplate) {
        CalendarOperations calendarOperations = new CalendarOperations();
    ...
    private DhPlaceCategories getMainCategory(String mainCategoryId, @NonNull MongoTemplate mongoTemplate) {
        Query queryFindCategoryWithId = new Query(Criteria.where(AppConstants.PLACE_MAIN_CATEGORY_ID).is(mainCategoryId));
